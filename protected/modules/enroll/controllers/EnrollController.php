...
class EnrollController extends CController
{
	...
	public function actionAdmin()
	{
		...
		$pages=new CPagination(Enroll::model()->count($criteria));
		$pages->pageSize=50;
		$pages->applyLimit($criteria);

		$sort=new CSort('Enroll');
		$sort->defaultOrder='createTime DESC';
		$sort->applyOrder($criteria);

		$models=Enroll::model()->findAll($criteria);
	...
	public function loadModel($id=null,$new=false)
	{
		...
		{
			if($new)
				$this->_model=new Enroll();
			if($id!==null || isset($_GET['id']))
				$this->_model=Enroll::model()->findbyPk($id!==null ? $id : $_GET['id']);
			if($this->_model===null)
