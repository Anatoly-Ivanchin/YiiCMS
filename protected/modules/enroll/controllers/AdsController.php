...
class AdsController extends CController
{
	...
	public function actionAdmin()
	{
		...
		$pages=new CPagination(EnrollAds::model()->count($criteria));
		$pages->pageSize=Yii::app()->params['pageSizes']['admin'];
		$pages->applyLimit($criteria);

		$sort=new CSort('EnrollAds');
		$sort->applyOrder($criteria);

		$models=EnrollAds::model()->findAll($criteria);
	...
	public function loadModel($id=null,$new=false)
	{
		...
		{
			if($new)
				$this->_model=new EnrollAds();
			if($id!==null || isset($_GET['id']))
				$this->_model=EnrollAds::model()->findbyPk($id!==null ? $id : $_GET['id']);
			if($this->_model===null)
